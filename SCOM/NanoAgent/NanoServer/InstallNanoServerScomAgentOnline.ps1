...
$installLogFileName = $env:TEMP + "\InstallLog"+$NanoServer+".txt"
$installStateFileName = $env:TEMP + "\InstallState"+$NanoServer+".txt"
...
$firewallOpened = 0
$agentNotPreviouslyInstalled = 0
$managementGroupRegistryDone = 0
$foldersCreated = 0
...
function CopyAndExpandCab([string]$SourceCabName, [string]$DestinationFolder)
{
    ...
    Invoke-Command -ComputerName $NanoServerFQDN -ScriptBlock {expand.exe $args[0] -F:* $args[1]} -ArgumentList $sourceCab,$DestinationFolder
...
function RemoveExpandedCabFolder()
{
    if((Test-Path $RTMCabExpandedFolder) -eq $true)
    {
        ...
        Invoke-Command -ComputerName $NanoServerFQDN -ScriptBlock {Remove-Item -Path $args[0] -Recurse} -ArgumentList $RTMCabExpandedFolder
    }
    if((Test-Path $UpdateCabExpandedFolder) -eq $true)
    {
        LogMessage "Removing the expanded update cab folder" $INFORMATION
        Invoke-Command -ComputerName $NanoServerFQDN -ScriptBlock {Remove-Item -Path $args[0] -Recurse} -ArgumentList $UpdateCabExpandedFolder
    }
}
...
LogMessage "Getting Management Server ID for registry settings" $INFORMATION
# Adding Management Group registry entries
try
{
	...
	$script:managementGroupRegistryDone = 1
}
catch
{
	$script:managementGroupRegistryDone = 0
...
if($script:registryChangesDone -eq 0)
{
	...
	try
	{
		...
		$VariableRegistry = $VariableRegistry -replace "{ManagementGroupName}", $ManagementGroupName
		$VariableRegistry = $VariableRegistry -replace "{ManagementServerFQDN}", $ManagementServerFQDN
		...
		$script = {			
            $InstallLocationLocal = "$env:SystemDrive\Program Files\Microsoft Monitoring Agent\Agent"
		...
		# Importing registry
		Invoke-Command -ScriptBlock $script -ComputerName $NanoServerFQDN -ErrorAction Stop
...
<#
    .SYNOPSIS 
      Installs the SCOM Nano Agent from the Nano Server. 
